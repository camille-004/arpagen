...
from preprocess import Vocab, one_hot_encode
from functions import CorpusTool
...
def plot_history(train_loss: List[float], val_loss: List[float], title: str = "Character Training Loss Plot"):
    ...
    plt.title(title)
    plt.legend()
    plt.savefig('char loss plot 50 20 35.pdf')
    
    plt.show()


def parserWords(text):
    text = text.replace('<BOS>', '').split('<EOS>')
    text = '. '.join([i.strip().capitalize() for i in text])
    
    return text
    
def parserChars(text):
    text = text.replace(" ", "")
...
if __name__ == "__main__":
    '''
    corpus = np.load("../data/corpus_characters.npy", allow_pickle=True)
    vocab = pickle.load(open("../data/vocab_characters.pkl", "rb"))
    rnn = RNN(corpus, vocab, n_hidden=512, n_layers=2)
        
    
    n_seqs, n_steps = 50, 20
    train_losses,  val_losses = train(
       rnn, corpus, epochs=35, _n_seqs=n_seqs, _n_steps=n_steps
    )
    torch.save(rnn, "../models/character_model 50 20 35.pkl")
      
    plot_history(train_losses, val_losses)
    '''
    
    corp_tool = CorpusTool()
    corp_tool.arpabet_reader("arpabetNew.json")
    

    
    #predicted_str = top_k_sample(brnn_model, prediction_type="phoneme", size=80, prime="<BOS>", top_k=5, cuda=True,)
    #generatedStrToOutput = corp_tool.RNN_out_to_str(predicted_str)
    #print("\n", generatedStrToOutput)
    
    
    #rnn_model = torch.load("../models/phoneme_model 50 20 20.pkl")
    #rnn_model = torch.load("../models/word_model 50 20 35.pkl")
    rnn_model = torch.load("../models/character_model 50 20 35.pkl")
    
    
    
    s = "l u f f <SPACE> y o u <SPACE> l u b b e r <SPACE> c r i e d" #alice thought she saw   what shouted hercules very   well known road shaking his   colin slowly sat up  luff you lubber cried
    primeString = ""
    #print(corp_tool.sentence_to_phonemes(s))
    
    #phonemeList = corp_tool.sentence_to_phonemes(s)
    #for each in phonemeList:
    #    for phoneme in each:
    #        primeString += phoneme + " "
    
    #primeString = primeString.rstrip(primeString[-1])
    #print(primeString)
    ...
    for i in range(10):
        flag = False
        predicted_str = top_k_sample(
                rnn_model,
                prediction_type="word",
                size=80, prime="<BOS> " + s, top_k=5, cuda=True,) #char # + primeString
        
        
        #print("\n", predicted_str) #just print here for word based

        #generatedStrToOutput = corp_tool.RNN_out_to_str(predicted_str)
       
        #print(parserWords(predicted_str),"\n")
        print(parserChars(predicted_str),"\n")
            
        #print("\n\n", generatedStrToOutput,"") if flag else print("\n\n", generatedStrToOutput)
    
    
                
        #lastTokenOrWord = predicted_str[predicted_str.rfind(" "):len(predicted_str)]
        
        #if lastTokenOrWord == " <SPACE>":
            #predicted_str = predicted_str[:predicted_str.rfind(" ")]
            #flag = True
