...
def split_input_target(padded_sentences: np.array) -> np.array:
    ...
    for sent in padded_sentences:
        _input = sent.copy()
        last_input_non_pad = np.argwhere(_input != PADDING_TOKEN)[-1]
        _input[tuple(last_input_non_pad.T)] = PADDING_TOKEN

        target = sent.copy()
        first_target_non_pad = np.argwhere(target != PADDING_TOKEN)[0]
        target[tuple(first_target_non_pad.T)] = PADDING_TOKEN
    ...
    padded_result = np.where(padded_result == 0, PADDING_TOKEN, padded_result)
